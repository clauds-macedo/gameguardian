import fetchPromoGamesIdsSteam from "../../modules/services/get_games_in_promotion";
import { Request, Response } from "express";
import axios from "axios";
import PromotionService from "../../modules/services/get_games_by_dev_steam";
...
class SteamController {
  ...
  async getPromotions(req: Request, res: Response) {
    try {
      const ids = await fetchPromoGamesIdsSteam();
  ...
  async getPromotionsByDeveloper(req: Request, res: Response) {
    ...
    try {
      const getGamesInPromotionByDevSteam = new PromotionService().promotionsByDevSteam
