...
export class AuthRepository extends DatabaseRepository<User> implements IAuthRepository {
  async login(requestDTO: AuthCredentialsDTO): Promise<User> {
    const { email, password } = requestDTO;
    const result = await auth().signInWithEmailAndPassword(email, password);
    return { email: result.user.email!, id: result.user.uid, name: result.user.displayName, photo: result.user.photoURL };
  }
  ...
  async register(requestDTO: RegisterDTO): Promise<User> {
    ...
    return { email: result.user.email!, id: result.user.uid, name, photo: result.user.photoURL };
  }
